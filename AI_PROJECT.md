...
- Problem: Agent/Hysteria communication or auth fails in production. Trigger: heartbeat, task payloads, local auth, Hysteria HTTP auth, panel outages, or public API URL changes. Rule: heartbeat updates latest metrics only, history comes from explicit snapshots, task payloads are objects, Hysteria auth supports POST, Agent survives panel outages, and local Agent auth controls new connections when the panel is unreachable.
