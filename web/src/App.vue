...
const pendingDeleteNodeOffline = computed(() => (pendingDeleteNode.value ? isNodeAgentOffline(pendingDeleteNode.value) : false))
const pendingDeleteNodeMessage = computed(() => {
  const nodeName = pendingDeleteNode.value?.name || '-'
  if (pendingDeleteNodeOffline.value) {
    return `节点 ${nodeName} 当前离线，将仅从面板移除。该节点下的用户副本会一并删除，其他节点上的同名用户不受影响。`
  }
  return `是否卸载并删除节点：${nodeName}。该节点下的用户副本会一并删除，其他节点上的同名用户不受影响。`
})
...
async function confirmDeleteNode() {
  ...
  await withBusyAction(
    ...
    shouldUninstallBeforeNodeDelete(node) ? `节点 ${node.name} 已卸载并删除` : `节点 ${node.name} 已从面板移除`,
  )
